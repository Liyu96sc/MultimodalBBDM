...
from runners.DiffusionBasedModelRunners.ImageCross_BBDMRunner import ImageCross_BBDMRunner
...
def parse_args_and_config():
    ...
    parser.add_argument('-c', '--config', type=str, default='./configs/MBBDM.yaml', help='Path to the config file')
    ...
    parser.add_argument('--image_cross', action='store_true', default=False, help='image cross')
    parser.add_argument('--save_top', action='store_true', default=False, help="save top loss checkpoint")

    parser.add_argument('--gpu_ids', type=str, default='3', help='gpu ids, 0,1,2,3 cpu=-1')
...
def DDP_run_fn(rank, world_size, config):
    ...
    if config.args.train:
        runner.train()
    elif config.args.image_cross:
        runner.img_cross_test()
    else:
        with torch.no_grad():
            runner.test()
    return
